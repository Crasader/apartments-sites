          <div id="sec-search-property">
            <select name="jumpMenu" id="jumpMenu" onchange="MM_jumpMenu('parent',this,0)" class="sec-search-list">
              ...
              <?php //foreach($properties as $Pkey => $Pvalue):
                    ...
                    $propertySelected = '';
                    if($property->getId() == $sf_params->get('id')){
                      $propertySelected = 'selected';
                    }
                    $strLinkURL = preg_replace('/\s/','',$property->getState()->getName()) ."/". preg_replace('/\s/','',$property->getCity()) ."/". preg_replace('/[\s\.]/','',$property->getName()) ."/Apartments/". $property->getId();
              ?>

              <?php /*<option <?php echo $propertySelected ?> value="<?php echo url_for('property/list?view=detail&id='.$Pkey); ?>"><?php echo $Pvalue ?></option>*/?>
              <option <?php echo $propertySelected ?> value="/<?php echo $strLinkURL; ?>"><?php echo $property->getName() ?></option>
              <?php endforeach; ?>
            </select>
          </div>
